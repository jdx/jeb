...
let request = (path, opts) => {
  ...
  .then(rsp => {
    log.debug(`http ${opts.method} ${uri} ${rsp.statusCode}`);
    return rsp;
  })
  .catch(err => {
    log.debug(`http ${opts.method} ${uri} ${err.statusCode}`);
    throw err;
  });
};
...
let getPackage = (name) => {
  ...
  let readJSONFile = f => fs.readFile(f).then(JSON.parse).catch(e => l.debug(e.message));
  let getPackageFromCache = name => readJSONFile(cachepath(name));

  let savePackageToCache = pkg => {
    let f = cachepath(pkg.name);
    l.debug('registry', `saving ${pkg.name} to ${f}`);
  ...
  let getPackageFromRegistry = (name, cached) => {
    ...
    if (cached) {
      ...
      l.debug(`fetching ${name} with etag`);
    } else {
      l.debug(`fetching ${name}`);
    }
    ...
    .catch(err => {
      ...
      l.debug(`returning ${name} from cache`);
      return cached;
    });
  };

  l.verbose(`fetching ${name}`);
...
let fetchPackageTarball = (pkg, version, dest) => {
  let task = new log.Task(`${chalk.cyan(pkg)} ${chalk.gray(version)}`);
  let tmpdir = path.join(config.tmpDir, uuid.v4());
  task.info(chalk.yellow('finding...'));
  return mkdirp(tmpdir)
  .then(() => getPackageVersion(pkg, version))
  .then(pkg => {
    task.info(chalk.yellow(`downloading...`));
